...
// Simulate fetching user data (replace with your DB logic)
async function getUserData(userId) {
    return new Promise((resolve, reject) => {
        ...
        db.query(query, [userId], (err, results) => {
            ...
            // Use profile photo if exists, otherwise fallback to default
            let photoPath;
...
router.get('/download/:userId', async (req, res) => {
    try {
        ...
        res.setHeader('Content-Disposition', 'attachment; filename="idcard.jpg"');
        res.send(finalBuffer);

    } catch (err) {
        console.error(err);
        res.status(500).send('Failed to generate ID card');
    }
});
